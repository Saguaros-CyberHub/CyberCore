...
router.get('/:courseId', instructorOnly, async (req, res) => {
  try {
    ...
    const courseResult = await query(`
      ...
      FROM cle_course c
      LEFT JOIN cle_course_enrollment e ON c.course_id = e.course_id AND e.status = 'active'
      LEFT JOIN cle_user_vm_assignment v ON c.course_id = v.course_id
...
router.patch('/:courseId', instructorOnly, async (req, res) => {
  try {
    const instructorId = req.user.userId;
    const { courseId } = req.params;
    const { course_name, description, code, is_active } = req.body;
    ...
    const updateResult = await query(`
      ...
      WHERE course_id = $5
      RETURNING *
    `, [course_name, description, code, is_active, courseId]);

    res.json({ success: true, course: updateResult.rows[0] });
  } catch (error) {
...
/**
 * DELETE /api/cle/courses/:courseId — Delete/archive a course
 */
router.delete('/:courseId', instructorOnly, async (req, res) => {
  try {
    const instructorId = req.user.userId;
    const { courseId } = req.params;
