...
-- ============================================================================
-- CLE USER VM ASSIGNMENTS TABLE
-- ============================================================================
-- Many-to-many: Students/faculty assigned to specific VMs
-- References cybercore_vm_instance for VM access
-- Each student gets their own lane (VXLAN isolation)
CREATE TABLE IF NOT EXISTS cle_user_vm_assignment (
  assignment_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL,
  vm_instance_id UUID NOT NULL,
  course_id UUID REFERENCES cle_course(course_id) ON DELETE SET NULL,
  lane_id UUID,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  access_level VARCHAR(50) DEFAULT 'user' CHECK (access_level IN ('user', 'admin', 'readonly')),
  status VARCHAR(50) DEFAULT 'provisioning' CHECK (status IN ('provisioning', 'active', 'suspended', 'expired', 'deleted')),
  expiration_date TIMESTAMP,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  UNIQUE(user_id, vm_instance_id)
);

CREATE INDEX IF NOT EXISTS idx_cle_vm_assignment_user ON cle_user_vm_assignment(user_id);
CREATE INDEX IF NOT EXISTS idx_cle_vm_assignment_vm ON cle_user_vm_assignment(vm_instance_id);
CREATE INDEX IF NOT EXISTS idx_cle_vm_assignment_course ON cle_user_vm_assignment(course_id);
CREATE INDEX IF NOT EXISTS idx_cle_vm_assignment_status ON cle_user_vm_assignment(status);
CREATE INDEX IF NOT EXISTS idx_cle_vm_assignment_lane ON cle_user_vm_assignment(lane_id);


-- ============================================================================
-- CLE COURSE VM ASSIGNMENTS TABLE
-- ============================================================================
-- Links courses to their required/recommended VMs
-- References cybercore_vm_instance for actual VMs
CREATE TABLE IF NOT EXISTS cle_course_vm_assignment (
  course_assignment_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  course_id UUID NOT NULL REFERENCES cle_course(course_id) ON DELETE CASCADE,
  vm_instance_id UUID NOT NULL,
  assignment_type VARCHAR(50) CHECK (assignment_type IN ('required', 'recommended', 'optional')),
  sequence_order INTEGER DEFAULT 0,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  UNIQUE(course_id, vm_instance_id)
);

CREATE INDEX IF NOT EXISTS idx_cle_course_vm_assignment_course ON cle_course_vm_assignment(course_id);
CREATE INDEX IF NOT EXISTS idx_cle_course_vm_assignment_vm ON cle_course_vm_assignment(vm_instance_id);
