...
function showWorkstationTemplateForm(tpl = null) {
  ...
  document.getElementById('wsTplProviderType').value  = tpl?.provider_type || '';
...
async function saveWorkstationTemplate() {
  ...
  const body = {
    ...
    provider_type: providerTypeVal || null,
  ...
  try {
    ...
    const typeNote = providerTypeVal ? '' : ' (VM type will be auto-detected from Proxmox)';
    statusEl.innerHTML = `<span style="color:#38a169;">Saved.${escHtml(typeNote)}</span>`;
...
async function wsTplVerify(id) {
  ...
  try {
    ...
    if (r.found) {
      ...
      if (typeEl && r.provider_type) typeEl.innerHTML = _providerTypeBadge(r.provider_type);
    } else {
      ...
      if (r.auto_disabled) {
        ...
        if (row) { const cb = row.querySelector('input[type=checkbox]'); if (cb) cb.checked = false; }
  ...
  try {
    ...
    container.innerHTML = `
      <table class="admin-table">
        <thead><tr>
          <th>Template</th><th>Key</th><th>Type</th><th>OS Family / VMID</th><th>Node</th>
        ...
        <tbody>
          ${templates.map(t => `
            <tr id="wsTplRow-${t.template_id}">
              ...
              <td id="wsTplType-${t.template_id}">${_providerTypeBadge(t.provider_type)}</td>
              ...
              <td style="white-space:nowrap;">
                <button class="btn btn-sm btn-outline" style="font-size:0.7rem;" onclick="wsTplVerify('${t.template_id}')">Test</button>
